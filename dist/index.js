  ...
  async getStabilityPoolBalance(account) {
    ...
    if (tokensRes.data) {
      const vusdBalances = tokensRes.data.map((token) => {
        if (_optionalChain([token, 'access', _17 => _17.data, 'optionalAccess', _18 => _18.content, 'optionalAccess', _19 => _19.dataType]) === "moveObject") {
          return Number(
...
async function buildWithdrawStabilityPoolTx(client, tx, sender, vusdAmount, recipient) {
  ...
  if (tokensRes.data) {
    ...
    tx.transferObjects([coin], _nullishCoalesce(recipient, () => ( sender)));
    return true;
  } else {
    return false;
  }
